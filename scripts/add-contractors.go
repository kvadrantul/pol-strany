...
func main() {
	...
	fmt.Println("Добавляем бригад в базу данных...")

	for _, contractor := range contractors {
		// Проверяем, существует ли пользователь
		var userID int64
		err := db.QueryRow("SELECT id FROM users WHERE telegram_id = ?", contractor.TelegramID).Scan(&userID)
		...
		if err == sql.ErrNoRows {
			...
			if err != nil {
				log.Printf("Ошибка создания пользователя %s: %v", contractor.Name, err)
				continue
			}
			
			userID, err = result.LastInsertId()
			if err != nil {
				log.Printf("Ошибка получения ID пользователя %s: %v", contractor.Name, err)
				continue
			}
		} else if err != nil {
			log.Printf("Ошибка проверки пользователя %s: %v", contractor.Name, err)
			continue
		}
		...
		var profileID int64
		err = db.QueryRow("SELECT id FROM contractor_profiles WHERE user_id = ?", userID).Scan(&profileID)
		...
		if err == sql.ErrNoRows {
			...
			if err != nil {
				log.Printf("Ошибка создания профиля для %s: %v", contractor.Name, err)
				continue
			}
			fmt.Printf("✅ Добавлен: %s (%s)\n", contractor.Name, contractor.Category)
		} else if err != nil {
			log.Printf("Ошибка проверки профиля для %s: %v", contractor.Name, err)
		} else {
			// Обновляем профиль
			_, err = db.Exec(
			...
			if err != nil {
				log.Printf("Ошибка обновления профиля для %s: %v", contractor.Name, err)
				continue
			}
	...
	fmt.Println("\nГотово! Всего бригад:", len(contractors))
}
