...
// Инициализация БД
// На Vercel используем Turso, локально - SQLite
let db;

function initDatabase() {
  if (!db) {
    // Проверяем, используется ли Turso (есть DATABASE_URL с libsql://)
...
router.get('/contractor/orders/:telegramId', async (req, res) => {
  try {
    const db = initDatabase();
    const user = await db.getUserByTelegramId(parseInt(req.params.telegramId));
    if (!user || user.role !== 'contractor') {
...
router.get('/contractor/pending-orders/:telegramId', async (req, res) => {
  try {
    const db = initDatabase();
    const user = await db.getUserByTelegramId(parseInt(req.params.telegramId));
    if (!user || user.role !== 'contractor') {
